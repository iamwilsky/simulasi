...
const ResultsWhatsAppShare: React.FC<ResultsWhatsAppShareProps> = ({
  ...
  const generateWhatsAppMessage = (userName: string, userPhone: string) => {
...
Terima kasih!`;
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (userName.trim() && userPhone.trim()) {
      generateWhatsAppMessage(userName, userPhone);
      setIsOpen(false);
      setUserName("");
      setUserPhone("");
    }
    ...
    <div className="flex justify-center pt-2">
      <Dialog open={isOpen} onOpenChange={setIsOpen}>
        <DialogTrigger asChild>
          <Button
            className="hover-lift bg-[#002C5F] hover:bg-[#003984] text-white border-0 shadow-lg shadow-[#002C5F]/20 px-8 py-4 rounded-2xl font-bold uppercase tracking-widest text-[11px] transition-all duration-300 hover:shadow-xl hover:scale-[1.02] active:scale-[0.98] group"
            size="lg"
          >
